...
const validateRequest = (req, res, next) => {
    ...
    // If ALLOW_NO_ORIGIN=true then bypass domain whitelist and API key checks.
...
// If ALLOW_NO_ORIGIN=true, enable permissive CORS for development (allows any origin).
// This sets Access-Control-Allow-* headers and responds to preflight OPTIONS requests.
// WARNING: this is intentionally permissive and should NOT be enabled in production.
if (process.env.ALLOW_NO_ORIGIN === 'true') {
    app.use((req, res, next) => {
        const origin = req.get('origin') || '*';
        // Echo origin when available, otherwise allow any origin
    ...
    // CORS intentionally disabled — expect gateway/reverse proxy to handle it.
}
